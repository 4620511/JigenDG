...
from data.JigsawLoader import JigsawDataset, JigsawTestDataset, _dataset_info, get_split_dataset_info
...
dataset_std = {
    mnist: (0.30280363, 0.30280363, 0.30280363),
...
dataset_mean = {
    mnist: (0.13909429, 0.13909429, 0.13909429),
...
def get_train_dataloader(args, patches):
    ...
    for dname in dataset_list:
        name_train, name_val, labels_train, labels_val = get_split_dataset_info(
            join(dirname(__file__), "txt_lists", "%s_train.txt" % dname), args.val_size
        )
...
def get_target_jigsaw_loader(args):
    ...
    name_train, _, labels_train, _ = get_split_dataset_info(
        join(dirname(__file__), "txt_lists", "%s_train.txt" % args.target), 0
    )
