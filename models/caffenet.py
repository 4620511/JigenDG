...
class AlexNetCaffe(nn.Module):
    ...
    def get_params(self, base_lr):
        return [
            {"params": self.features.parameters(), "lr": 0.0},
            {
                "params": chain(
...
def caffenet_fc7(jigsaw_classes, classes):
    ...
    del state_dict["classifier.fc7.bias"]
    model.load_state_dict(state_dict, strict=False)
    nn.init.xavier_uniform_(model.jigsaw_classifier.fc8.weight, 0.1)
    nn.init.constant_(model.jigsaw_classifier.fc8.bias, 0.0)
    nn.init.xavier_uniform_(model.class_classifier.fc8.weight, 0.1)
    nn.init.constant_(model.class_classifier.fc8.bias, 0.0)
    return model
