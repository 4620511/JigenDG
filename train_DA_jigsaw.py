...
def get_args():
    ...
    parser.add_argument(
        "--network",
        choices=model_factory.nets_map.keys(),
...
class Trainer:
    def __init__(self, args, device):
        ...
        print(
            "Dataset size: train %d, target jig: %d, val %d, test %d"
            % (
                len(self.source_loader.dataset),
                len(self.target_jig_loader.dataset),
    ...
    def _do_epoch(self):
        ...
        for it, (source_batch, target_batch) in enumerate(
            zip(self.source_loader, itertools.cycle(self.target_jig_loader))
        ):
            (data, jig_l, class_l), d_idx = source_batch
            data, jig_l, class_l, d_idx = (
                data.to(self.device),
                jig_l.to(self.device),
                class_l.to(self.device),
            ...
            loss = (
                class_loss
                + jigsaw_loss * self.jig_weight
                + target_jigsaw_loss * self.target_weight
                + target_entropy_loss * self.target_entropy
            )
